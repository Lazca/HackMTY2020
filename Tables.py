...
def getTableArrangement(boundary, minDist, tableRadius, personWidth):
    tables = []
    length = 2*minDist + tableRadius + personWidth
    ...
    def addTable(x, y):
        if not inBound(boundary, x, y):
            ...
            for i in range(6):
                newX = length*math.cos(math.pi*i/3)
                newY = length*math.sin(math.pi*i/3)
...
if __name__ == "__main__":
    tables = getTableArrangement((0,0,7.5,5), 1.5, 1, 1)
